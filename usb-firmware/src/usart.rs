...
static STAGING_FIGHTSTICK: Mutex<RefCell<FightstickDescriptor>> = Mutex::new(RefCell::new(FightstickDescriptor([0, 0, 0, 0])));
...
pub fn setup_usart(cs: &CriticalSection, usart: USART1, portd: &PORTD) {
  ...
  usart.ucsr1c.write(|w| w
    .umsel1().usart_async() // Async USART
    .upm1().disabled() // No Parity
    .usbs1().stop1() // 1 Stop Bit
    .ucsz1().chr8()
  );
  usart.ucsr1b.write(|w| w
    .rxen1().set_bit()
    .txen1().set_bit()
    .rxcie1().set_bit()
  );
